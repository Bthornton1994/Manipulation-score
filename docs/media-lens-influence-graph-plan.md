...
Limits (config in `worker/config.js`, enforced in `server.js`, `prepare.js`, and `analyze.js`): request body 512 KB; prepared text 60,000 chars; 200 spans; 5 analyses per minute per worker (`MEDIA_LENS_MAX_ANALYSES_PER_MINUTE`); 160 Jev calls per analysis (`MEDIA_LENS_MAX_JEV_CALLS_PER_ANALYSIS`); 8 s per Jev call; 15 s whole-pipeline timeout (`MEDIA_LENS_PER_ANALYSIS_TIMEOUT_MS`, aborts in-flight Jev retries via a shared `AbortController`); live URL caps 5/minute/worker, 2/host/minute, 1 concurrent fetch when enabled; ESTIMATED TypeSafe monthly budget warn $20 / stop $30 (Jev only, not verified billing). Operator detail: `docs/media-lens-ops-runbook-v2.md` §3 and `media-lens/README.md`. Exceeding a limit yields HTTP 413/429 or a graph with a single `oversized_input` or `engine_unavailable` abstention rather than a partial result.
