...
`MEDIA_LENS_JEV_SHADOW` does not ask `media-lens-narrow.v1`. That comparison is a separate flag, documented in `docs/jev-usage-lab/10-narrow-shadow-execution.md`. It is default off and is not an accuracy result. The 2026-09-22 PT approval on that page leaves the per-call estimate unset, so the narrow network stays off.
