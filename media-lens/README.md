...
### Worker fixes in the 2026-09 release (PRs #146, #145, #149)
...
- **#149, unreadable budget record.** If the budget file exists but cannot be read or fails the schema check, live Jev fails closed before any provider call with "The TypeSafe ESTIMATED budget record could not be read or updated, so no live Jev analysis was performed." That state lasts until the worker restarts with a readable, valid file. A missing file still starts the month at zero. A failed write keeps the new calls in memory only, and a later read of the older file can replace them, so the file and its directory must stay writable by the worker user (checks in `docs/media-lens-frontend-deployment.md`).
