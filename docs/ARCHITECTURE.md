...
If only `CACHE_NAME` changes but the `?v=` query stays stale, users can see the previous design until caches expire.
...
5. **Deploy** — Merge to `main`; GitHub Actions runs tests then deploys to GitHub Pages (see `README.md` and `docs/HTTPS.md`).
