...
`MEDIA_LENS_JEV_SHADOW` does not ask `media-lens-narrow.v1`. That comparison is a separate flag, documented in `docs/jev-usage-lab/10-narrow-shadow-execution.md`. It is default off and is not an accuracy result. The 2026-09-22 PT record on that page stores 0.002 as an ESTIMATED planning constant only. The runtime env stays unset, so the narrow network stays off.
