...
2. Keep a walked block, including `byline_meta`, when its normalized text exactly matches a Trafilatura line, contains a Trafilatura line (`<br>`-split paragraphs), is contained in a Trafilatura line, or matches after a leading list marker (`-`, `*`, or `•`) is removed.
