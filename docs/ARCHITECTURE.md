...
| Service worker cache | `CACHE_NAME` in `service-worker.js` | Bumped when cached assets change (currently `clarity-v33`) |

Keep `METHODOLOGY_VERSION`, `methodology.html`, `changelog.html`, and trust-page copy aligned when changing screening behavior.
...
Safety and scoring share `text-normalize.js` for consistent Unicode handling.
...
`PILOT_SUPPRESS_NUMERIC_SCORE = true` in `scoring.js`. The engine still computes a 0–100 score internally; `app.js` shows pattern bands (Low / Moderate / High) without the numeric value.
...
`service-worker.js` caches the app shell, core modules, fonts, and vendored Tesseract assets. After the first visit, the analyzer works offline. When a new service worker activates, `app.js` shows a "Refresh now" banner.
...
Re-enable by setting `BETA_OCR_ENABLED = true` and verifying OCR regression tests.
...
| `audit-v031.test.js`, `audit-v032.test.js` | Release gate tests for v0.3.1 / v0.3.2 |
...
- **Version drift:** update `METHODOLOGY_VERSION`, `changelog.html`, `methodology.html`, and bump `CACHE_NAME` when shipping logic changes.
