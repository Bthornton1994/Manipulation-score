...
| `MEDIA_LENS_JEV_SHADOW_NARROW_ESTIMATED_USD_PER_CALL` | approved operating state is unset, which keeps real narrow calls at zero; not the production 0.002 planning rate; not $0.042/MTok; planning `T_max` is in `docs/jev-usage-lab/narrow-shadow-request-bound.v1.json` and is not a dollar estimate; a later live call needs a separate owner authorization |
