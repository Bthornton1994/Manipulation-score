...
- Deploy: `.github/workflows/ci.yml` runs tests, then on `main` rsyncs the entire repo (minus `.git`, `.github`, `.venv`, `.cursor`, `tests`, `_site`) to GitHub Pages. Anything committed to the root ships to production on merge. This plan adds `media-lens/` to the rsync exclude list so nothing new is published (draft only, no deploy).
  - Dated note, 2026-09-25: the rsync step no longer exists. The `deploy` job in `.github/workflows/ci.yml` runs `node scripts/build-pages-site.js _site`, which copies only an explicit allowlist of Clarity site files (`PAGES_ROOT_FILES`, plus `fonts/` and `vendor/`) and refuses `docs/`, `media-lens/`, `tests/`, `scripts/`, the READMEs, and agent-control files (`PAGES_FORBIDDEN_NAMES`). `docs/*.md`, including this plan, is not published to Pages. Media Lens is served only by the operator host `ml-jev.manipulationscore.com` through `media-lens/deploy/Caddyfile` (see `docs/media-lens-frontend-deployment.md`). The rsync text above and in later sections is the original plan record.
