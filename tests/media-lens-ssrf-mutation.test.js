...
test('mutation: dropping well-known NAT64 extraction still blocks via unknown-NAT64 catch-all', async () => {
  ...
  assert.equal(mutant.classifyIp('64:ff9b::808:808').disposition, 'block');
});
...
test('mutation: dropping local-use NAT64 extraction still blocks via unknown-NAT64 catch-all', async () => {
  ...
  assert.equal(loopback.disposition, 'block');
  assert.equal(loopback.reason, 'block_nat64_unknown');
  assert.equal(mutant.classifyIp('64:ff9b:1:808:8:800::').disposition, 'block');
});
...
test('mutation: dropping ISATAP detection still blocks via residual ISATAP fail-closed', async () => {
  ...
  assert.equal(classifyIp('2001:470:1:2:0:5efe:808:808').reason, 'block_isatap');
  assert.equal(classifyIp('2001:470:1:2:100:5efe:808:808').reason, 'block_isatap');
  assert.equal(classifyIp('2001:470:1:2:300:5efe:808:808').reason, 'block_isatap');
  const mutant = await loadMutatedAddressPolicy((src) =>
    src.replace(
  ...
  for (const ip of [
    '2001:470:1:2:0:5efe:808:808',
    '2001:470:1:2:100:5efe:808:808',
    '2001:470:1:2:200:5efe:808:808',
    '2001:470:1:2:300:5efe:808:808'
  ]) {
    const publicIsatap = mutant.classifyIp(ip);
    assert.equal(publicIsatap.disposition, 'block', ip);
    assert.equal(publicIsatap.reason, 'block_isatap', ip);
    assert.equal(publicIsatap.embeddedIPv4, '8.8.8.8', ip);
  }
...
test('mutation: dropping extra NAT64 would allow private last-32 embeddings as public IPv6', async () => {
  ...
  assert.equal(classifyIp('2001:470:1::cb00:7107').reason, 'block_documentation_via_nat64_extra');
  assert.equal(classifyIp('2001:67c:27e4:64:ff:9b:cb00:7107').reason, 'block_documentation_via_nat64_extra');
